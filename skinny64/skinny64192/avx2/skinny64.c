...
void encrypt_64blocks(u256 x[16], u256 rk[NROUNDS][8]) {

    int i;
    int j;
    u256 tmp[4];

    u256 rc = _mm256_set_epi64x(
        0xFF000000FF0000ull,
        0xFF000000FF0000ull,
        0xFF000000FF0000ull,
        0xFF000000FF0000ull
...
void key_schedule(const unsigned char *k, u256 rk[NROUNDS][8]) {
    int i;
    int j;
    u256 tk1[16];
    u256 tk2[16];
    ...
    unsigned char *tmp_key = malloc(48);
    ...
    for(i = 0; i < 4; i++) {
        memcpy(tmp_key + 8*i, k + 16, 8);
    }

    pack_key(tk3, tmp_key);

    for(j = 0; j < NROUNDS; j++) {

        // Extract round key
        for(i = 0; i < 8; i++) {
            rk[j][i] = XOR(tk1[i], XOR(tk2[i], tk3[i]));
        }
        ...
        if( (RC[j] >> 5) & 1 ) rk[j][6] = XOR(rk[j][6], rc);
        if( (RC[j] >> 4) & 1 ) rk[j][7] = XOR(rk[j][7], rc);
        if( (RC[j] >> 3) & 1 ) rk[j][0] = XOR(rk[j][0], rc);
        if( (RC[j] >> 2) & 1 ) rk[j][1] = XOR(rk[j][1], rc);
        if( (RC[j] >> 1) & 1 ) rk[j][2] = XOR(rk[j][2], rc);
        if( (RC[j] >> 0) & 1 ) rk[j][3] = XOR(rk[j][3], rc);
        ...
        for(i = 0; i < 4; i++) {
            tmp[0 + i] = XOR(
                _mm256_shuffle_epi8(tk1[ 8   + i], _mm256_set_epi8(30,28,0xff,0xff,26,24,0xff,0xff,22,20,0xff,0xff,18,16,0xff,0xff,14,12,0xff,0xff,10,8,0xff,0xff,6,4,0xff,0xff,2,0,0xff,0xff)),
            ...
            tmp[4 + i] = XOR(
                _mm256_shuffle_epi8(tk1[12   + i], _mm256_set_epi8(30,0xff,0xff,31,26,0xff,0xff,27,22,0xff,0xff,23,18,0xff,0xff,19,14,0xff,0xff,15,10,0xff,0xff,11,6,0xff,0xff,7,2,0xff,0xff,3)),
        ...
        for(i = 0; i < 4; i++) {
            tmp[0 + i] = XOR(
                _mm256_shuffle_epi8(tk2[ 8   + i], _mm256_set_epi8(30,28,0xff,0xff,26,24,0xff,0xff,22,20,0xff,0xff,18,16,0xff,0xff,14,12,0xff,0xff,10,8,0xff,0xff,6,4,0xff,0xff,2,0,0xff,0xff)),
            ...
            tmp[4 + i] = XOR(
                _mm256_shuffle_epi8(tk2[12   + i], _mm256_set_epi8(30,0xff,0xff,31,26,0xff,0xff,27,22,0xff,0xff,23,18,0xff,0xff,19,14,0xff,0xff,15,10,0xff,0xff,11,6,0xff,0xff,7,2,0xff,0xff,3)),
        ...
        for(i = 0; i < 4; i++) {
            tmp[0 + i] = XOR(
                _mm256_shuffle_epi8(tk3[ 8   + i], _mm256_set_epi8(30,28,0xff,0xff,26,24,0xff,0xff,22,20,0xff,0xff,18,16,0xff,0xff,14,12,0xff,0xff,10,8,0xff,0xff,6,4,0xff,0xff,2,0,0xff,0xff)),
                _mm256_shuffle_epi8(tk3[12   + i], _mm256_set_epi8(0xff,0xff,28,29,0xff,0xff,24,25,0xff,0xff,20,21,0xff,0xff,16,17,0xff,0xff,12,13,0xff,0xff,8,9,0xff,0xff,4,5,0xff,0xff,0,1))
            );
            tmp[4 + i] = XOR(
                _mm256_shuffle_epi8(tk3[12   + i], _mm256_set_epi8(30,0xff,0xff,31,26,0xff,0xff,27,22,0xff,0xff,23,18,0xff,0xff,19,14,0xff,0xff,15,10,0xff,0xff,11,6,0xff,0xff,7,2,0xff,0xff,3)),
                _mm256_shuffle_epi8(tk3[ 8   + i], _mm256_set_epi8(0xff,31,29,0xff,0xff,27,25,0xff,0xff,23,21,0xff,0xff,19,17,0xff,0xff,15,13,0xff,0xff,11,9,0xff,0xff,7,5,0xff,0xff,3,1,0xff))
            );
        }
        ...
        tk3[2] = tmp[1];
        tk3[1] = tmp[0];
        tk3[0] = XOR(tmp[0], tmp[3]);
        tk3[7] = tmp[6];
        tk3[6] = tmp[5];
        tk3[5] = tmp[4];
        tk3[4] = XOR(tmp[4], tmp[7]);
